...
#class ParallelizationInference(nn.Module):

 #   def __init__(self, args,RNN_TYPES):
 #       super(ParallelizationInference, self).__init__()
...
class MultiTurnInference(nn.Module):
    ...
    def __init__(self, args,RNN_TYPES):
        ...
        if "c" in args.matching_order:
            self.cat_linear= nn.Linear(args.cat_num*2*args.hidden_size,2*args.hidden_size)
            #self.cat_linear= nn.Linear(2*args.hidden_size,args.hidden_size)
        if "s" in args.matching_order:
            self.sub_linear= nn.Linear(2*args.hidden_size, 2*args.hidden_size)
        if "m" in args.matching_order:
            self.mul_linear= nn.Linear(2*args.hidden_size,2*args.hidden_size)
            ...
            if  self.args.use_bilstm==True:
                if self.args.use_conv==True:
                    self.conv =nn.Conv2d(in_channels=len(args.matching_order), 
                                         out_channels=1, kernel_size=(1,2), 
