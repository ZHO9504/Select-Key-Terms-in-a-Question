...
def load_data(path):
    ...
    for line in open(path, 'r', encoding='utf-8'):
        #print ("#######################################")
        #print ("load_data line",line)
        if path.find('race') < 0 or np.random.random() < 0.6:
            data.append(Example(json.loads(line)))
    print('Load %d examples from %s... use %d seconds' % (len(data), path, time.time()-start_time))
    return data
...
def gen_race_vocab(data):
    ...
    for ex in data:
        cnt += Counter(re.split('(\W+)',ex.passage))
        cnt += Counter(re.split('(\W+)',ex.question))
        cnt += Counter(re.split('(\W+)',ex.choice))
...
def build_vocab(data=None):
    ...
    if os.path.exists('./data/vocab'):
        ...
        for ex in data:
            cnt += Counter(re.split(r'(\W+)',ex.passage.strip()))
            cnt += Counter(re.split(r'(\W+)',ex.question.strip()))
            cnt += Counter(re.split(r'(\W+)',ex.choice.strip()))
    ...
    if os.path.exists('./data/ner_vocab'):
        ...
        cnt = Counter()
        for ex in data:
            cnt += Counter(ex.d_ner) + Counter(ex.q_ner)+ Counter(ex.c_ner)
        for key, val in cnt.most_common():
    ...
    if os.path.exists('./data/rel_vocab'):
        ...
        for ex in data:
            ...
            #print (ex.p_q_rel)
            #print(ex.d_ner)
            cnt += Counter(ex.p_q_rel) + Counter(ex.p_c_rel)  + Counter(ex.q_p_rel)  + Counter(ex.q_c_rel)  + Counter(ex.c_p_rel)  + Counter(ex.c_q_rel) 
        for key, val in cnt.most_common():
            if key: 
                print(key)
                print(str(key))
                rel_vocab.add(key)
        print('REL vocabulary size: %d' % len(rel_vocab))
        writer = open('./data/rel_vocab', 'w', encoding='utf-8')
        writer.write('\n'.join(rel_vocab.tokens()))
        writer.close()
...
if __name__ == '__main__':
    ...
    #dev_data = load_data('./data/mnli_train.txt-processed_11_26.json')
    #test_data = load_data('./data/cloze_test_ALL_test-processed.json')
