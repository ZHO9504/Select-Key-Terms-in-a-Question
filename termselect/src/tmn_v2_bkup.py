...
class MyModel(nn.Module):
    ...
    def forward(self, p, p_pos, p_ner, p_mask, q, q_pos, q_ner, q_mask, c,c_pos,c_ner, c_mask,\
        ...
        p_concat_feature = torch.cat([p_hiddens,p_q_weighted_hiddens,p_c_weighted_hiddens],2)
        p_sub_feature =  (p_hiddens -p_q_weighted_hiddens)*(p_hiddens - p_c_weighted_hiddens)
        p_mul_feature = p_hiddens*p_q_weighted_hiddens*p_c_weighted_hiddens

        p_mfeature = {"c":p_concat_feature, "s":p_sub_feature, "m":p_mul_feature }
        dim = p_hiddens.size()
        p_init_mem = torch.zeros(dim[0],dim[1],dim[2]).float().cuda()  #zero mem
        p_infer_emb,self.p_mem_list, self.p_mem_gate_list = self.mtinfer(p_mfeature,p_mask,init_mem=p_init_mem,x_order=self.args.matching_order)
        ...
        q_pc_cat = torch.cat([q_hiddens,q_p_weighted_hiddens,q_c_weighted_hiddens],2)
        q_pc_cat_weight,q_pc_cat_weighted_hiddens = self.hidden_match(q_pc_cat,q_pc_cat,q_mask)
        if self.args.dropout_att_score > 0:
            q_pc_cat_weight = nn.functional.dropout(q_pc_cat_weight, p=self.args.dropout_att_score, training= self.training)
        matched_q = q_pc_cat_weight.bmm(q_hiddens)
        ...
        c_pq_cat = torch.cat([c_hiddens,c_p_weighted_hiddens,c_q_weighted_hiddens],2)
        c_pq_cat_weight,c_pq_cat_weighted_hiddens = self.hidden_match(c_pq_cat,c_pq_cat,c_mask)
        if self.args.dropout_att_score > 0:
            c_pq_cat_weight = nn.functional.dropout(c_pq_cat_weight, p=self.args.dropout_att_score, training= self.training)
        matched_c = c_pq_cat_weight.bmm(c_hiddens)
