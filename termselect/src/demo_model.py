...
class Model:
    ...
    def evaluate(self, dev_data, debug=False, eval_train=False,is_paint=False):
        ...
        for batch_input in self._iter_data(dev_data):
            ...
            feed_input[-1]=0
            yy = batch_input[-1].data.cpu().numpy()
            ...
            gold +=[ [int(label) for label in labels ] for labels in yy]
            assert(len(list(pred_proba)) == len(gold))
        problist = prediction
        sums = 0
        num = 0
        truth=[]
    ...
    def predict(self, test_data,demo = False, outputpath = './data/demo_output'):
        # DO NOT SHUFFLE test_data
        self.network.eval()
        writer = open(outputpath, 'w', encoding='utf-8')
        correct, total, prediction, gold, problist= 0, 0, [], [], []
        for batch_input,batch_data in self._iter_data(test_data):
            feed_input = [x for x in batch_input[:-1]]
            ...
            for i, ex in enumerate(batch_data):
                if demo:
                    label = prediction[i-1][:lengths[i-1]].numpy()
                    question = batch_data[i - 1].question
                    choice = batch_data[i - 1].choice
                    #writer.write(choice + ' ||| ' + question + ' ||| ' + str(label))
                    #print(question)
                    #print(label)
                    question_words = str(question).split()
                    
                    qwords = ''
                    #print(labels)
                    for i in range(len(label)):
                        if int(label[i]) == 1:
                            qwords += question_words[i]+' '
                    #print(qwords)
                    #print('\n\n')
                    #print(choice + ' ||| ' + question + ' ||| ' + str(label) + ' ||| ' + qwords)
                    #print(question)
                    import re
                    question = re.sub('[^\w|\s]|\d',' ',question)
                    question = re.sub('\s+',' ',question)
                    writer.write(choice + ' ||| ' + question.strip() + ' ||| ' + qwords)
                    #writer.write(str(gold[i]))
                    writer.write('\n')
        #return prediction
