...
def batchify(batch_data):
    ...
    q, q_mask,q_position = _to_indices_and_mask([ex.q_tensor for ex in batch_data],mx_len = makelen)
    q_pos = _to_indices_and_mask([ex.q_pos_tensor for ex in batch_data], need_mask=False,mx_len = makelen)
    q_ner = _to_indices_and_mask([ex.q_ner_tensor for ex in batch_data], need_mask=False,mx_len = makelen)
    q_p_relation = _to_indices_and_mask([ex.q_p_relation for ex in batch_data], need_mask=False,mx_len = makelen)
    q_c_relation = _to_indices_and_mask([ex.q_c_relation for ex in batch_data], need_mask=False,mx_len = makelen)
    ...
    q_f_tensor = _to_feature_tensor([ex.q_features for ex in batch_data],mx_len = makelen)
