...
class MyModel(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.p_layer_norm = layers.LayerNorm(2*self.args.hidden_size)
        self.q_layer_norm = layers.LayerNorm(2*self.args.hidden_size)
        self.c_layer_norm = layers.LayerNorm(2*self.args.hidden_size)
        ...
        self.res_dropout = Dropout(args.dropout_residual)
    ...
    def forward(self, p, p_pos, p_ner, p_mask, q, q_pos, q_ner, q_mask, c,c_pos,c_ner, c_mask,\
        ...
        #layer_norm_q = self.q_layer_norm(q_hiddens +  self.res_dropout(matched_q))
        layer_norm_q = self.q_layer_norm(q_hiddens +  matched_q)
        ...
        #layer_norm_p = self.p_layer_norm(p_hiddens+ self.res_dropout (matched_p))
        layer_norm_p = self.p_layer_norm(p_hiddens+ matched_p)
        ...
        #_,c_p_weighted_hiddens = self.hidden_match(c_hiddens,layer_norm_p,p_mask)
        #_,c_q_weighted_hiddens = self.hidden_match(c_hiddens,layer_norm_q,q_mask)
        ...
        #layer_norm_c = self.c_layer_norm(c_hiddens+ self.res_dropout (c_infer_emb))
        ...
        _,matched_q_self = self.q_self_attn(layer_norm_q,q_mask) 
        _,matched_p_self = self.q_self_attn(layer_norm_p,p_mask) 
        #c_infer_hidden_ave = layers.ave_pooling(layer_norm_c,c_mask)
        #c_infer_hidden_max = layers.max_pooling(layer_norm_c)

        #import pdb
        #pdb.set_trace()
        infer_linear = self.c_infer_linear(torch.cat([c_infer_hidden_ave,c_infer_hidden_max,matched_q_self,matched_p_self],-1)) 
